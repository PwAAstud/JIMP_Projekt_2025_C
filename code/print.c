...
void pritplik(wczytanie* writ, int t, int b, int cuts){
    ...
    if (plik == NULL) {
        perror("Nie można otworzyć pliku wynik.csrrg");
        return;
    }
    fprintf(plik, "%ln\n", cuts );
    fprintf(plik, "%s\n", writ->linia1);
...
node* init_node(long id, long n) {
    node* new_node = (node*)malloc(sizeof(node));  // alokacja pami�ci na w�ze�
    if (new_node == NULL) {
    ...
    new_node->n = n;    // ustawienie liczby po��cze�
    new_node->conetion = (node**)malloc(memoryJump * sizeof(node*));  // alokacja pami�ci na po��czenia
    if (new_node->conetion == NULL) {
        printf("Blad alokacji pamieci na polaczenia!\n");
        free(new_node);  // zwolnienie wcze�niej zaalokowanej pami�ci
        exit(1);
    }

    // Inicjalizacja po��cze� jako NULL
    for (long i = 0; i < n; i++) {
